...
type WebSocketController struct {
	Hub          *hub.Hub
	GrupoService *servicegrupo.GrupoService // Agrega el servicio de grupos
}

// NewWebSocketController crea un nuevo controlador de WebSocket
func NewWebSocketController(h *hub.Hub, gs *servicegrupo.GrupoService) *WebSocketController {
	return &WebSocketController{Hub: h, GrupoService: gs}
}
...
func (c *WebSocketController) WebSocketChat(conn *websocket.Conn) {
	...
	// Get the user ID from claims. The claim key is "Id" (capitalized).
	userIDStr, ok := claims["id"].(string)
	if !ok || len(strings.TrimSpace(userIDStr)) == 0 {
	...
	groupIDs, err := c.GrupoService.GetAllGruposByUsuarioIdToIds(idUser)
	if err != nil {
	...
	c.Hub.SubscribeUserToGroups(userIDStr, groupIDs)
